...
from example_dir import medium
from example_dir.medium import get_list_of_followings

DEST_PATH = os.path.join(os.path.dirname(__file__), 'data/{date}'.format(date=date.today()))
...
class FetchUserProfile(luigi.Task):
    ...
    # Process only one portion of the input (the input for this task contains all the users)
    def run(self):
...
class ExtractUserMetrics(luigi.Task):
    """
    Extract some metrics from a user profile file
    """
    file_number = luigi.Parameter()
    ...
    def run(self):
        with self.output().open('r') as in_file, self.output().open('w') as out_file:
            in_json = json.loads(in_file.read())
            output_json = {
                'root': []
            }
...
class MainPipelineTask(luigi:WrapperTask):
    def requires(self):
        ...
        fetch_user_profile = [FetchUserProfile(file_number=id_) for id_ in ids]
        extract_user_metrics = [ExtractUserMetrics(file_number=id_) for id_ in ids]
        fetch_user_followings = [FetchUserFollowings(file_number=id_) for id_ in ids]

        return base_tasks + fetch_user_profile + extract_user_metrics + fetch_user_followings
    ...
    def run(self):
        with self.output().open('w') as out_file:
            out_file.write('Pipeline is executed successfully!')
    
def get_part_files(dir_name):
    following_dir = os.path.join(DEST_PATH, dir_name)
    onlyfiles = [for f in os.listdir(following_dir) if f.endswith('.json')]
    return onlyfiles
...
def get_local_target(path):
    return luigi.LocalTarget(os.path.join(DEST_PATH, path))
...
def get_config_value(value, default=None):
    return luigi.configuration.get_config().get('example_dir', value, default)